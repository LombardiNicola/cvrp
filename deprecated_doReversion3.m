...
function [q, lq] = doReversion3(p, lp, w, c, i1, i2)
    ...
    if i2 ~= n || sum(lp==(i1+1))
        delta = n - lastEnd; 
        % traslo i punti, i valori di lq, i1 e i2
        q = [q((lastEnd+1):n); q(1:lastEnd)];
        lq = lq + delta;
        i1=i1+delta;
        i2=i2+delta;
    else
        % sono stati scambiati due estremi. Per come e' stata scritta la
        % funzione, al momento ci sono 2 lastPoint uguali, in particolare
        % uno dei due è l'ultimo
        lq(end) = n;
    end
    % check feasability
